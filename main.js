...
class PoemSelector extends HTMLElement {
    ...
    render() {
        ...
        this.shadowRoot.innerHTML = `
            <style>
                .container { display: flex; gap: 3rem; align-items: center; justify-content: center; margin-top: 3rem; }
                .card { background: white; padding: 3rem; border-radius: 20px; box-shadow: var(--shadow); flex: 1; cursor: pointer; transition: transform 0.2s, box-shadow 0.2s; text-align: center; }
                .card:hover { transform: translateY(-10px); box-shadow: var(--shadow-hover); }
                .vs { font-weight: bold; font-size: 2rem; color: var(--primary-color); }
                h3 { font-size: 1.5rem; margin-bottom: 1rem; }
                p { font-size: 1.1rem; line-height: 1.6; }
            </style>
            <h2 style="text-align:center;">마음에 드는 시를 선택하세요</h2>
            <div class="container">
                <div class="card" id="p1"><h3>${p1.title}</h3><p>${p1.body.substring(0, 150)}...</p></div>
                <div class="vs">VS</div>
                <div class="card" id="p2"><h3>${p2.title}</h3><p>${p2.body.substring(0, 150)}...</p></div>
            </div>
        `;
